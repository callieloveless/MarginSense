# job-photos Specification

## Purpose
Holding a job's photos: capturing, storing, showing, captioning, and deleting them as
tenant-isolated assets. The bytes live outside Postgres, so this capability owns the rules that
keep them as private as any row — a `business_id`-prefixed key in a private bucket, an object
policy in the database, and short-lived signed URLs — plus the on-device preparation that keeps
a client's home coordinates off the wire (constitution §7). Uploading is an outer-layer **user
action** that commits directly (never a suggestion), and it emits `photo.uploaded` so tools can
later trigger from it. No AI: the vision tool that reads these photos is `photo-advisor`.
## Requirements
