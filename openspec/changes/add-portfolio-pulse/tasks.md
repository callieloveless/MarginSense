...
- [ ] 1.1 Add `aggregateProfitPerHour(jobs)` to the engine returning `{ aggregate, shortfall, signal,
...
- [ ] 1.2 Export it from `src/engine/index.ts` and surface it through `src/profit` for the dashboard.

## 2. Dashboard

- [ ] 2.1 Lead the dashboard with the pulse card (aggregate profit/hr, plain verdict scoped honestly
...
- [ ] 2.2 Reshape the ranked cards so each shows the job's own profit/hr + a signal chip + its note;
  keep the worst-first ordering and the "% of your year / % of your profit goal" figures.
- [ ] 2.3 Calm empty/NA states: no priced active jobs → the pulse's not-applicable state; draft jobs
...
- [ ] 2.4 Dashboard header per the prototype (business identity, settings entry) + the camera
...
- [ ] 3.1 `npm run typecheck`, `npx vitest run` (engine + dashboard), `npm run build`; phone-width
  dashboard pass.
- [ ] 3.2 **Isolation guard:** no schema/migration, `src/db/`, RLS, or `src/tools/` change; the
  aggregate reads the existing tenant-scoped active-estimate set — no new tenant surface.
- [ ] 3.3 Update `PROGRESS.md` R2 (portfolio-pulse) status; no new deferred live-infra items.
