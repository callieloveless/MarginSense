...
- [ ] 1.2 Add nullable `tool_run_id` (FK → `tool_runs`, `ON DELETE SET NULL`) to `suggestions`
      and `conversation_messages` in the schema
- [ ] 1.3 `npm run db:generate` → migration `0004`; hand-append RLS for `tool_runs` (enable +
...
- [ ] 1.4 `ToolRunsBackend` port — memory impl in `src/db/tenant.ts`, Drizzle impl in
...
- [ ] 1.5 Tenant-isolation test: cross-tenant read of `tool_runs` blocked; `business_id`
      stamped from the handle, never input

## 2. AI model port (`src/ai/`)

- [ ] 2.1 `src/ai/config.ts` — centralized model IDs (default `claude-opus-4-8`),
      effort/thinking defaults + AI settings; no magic constants elsewhere
- [ ] 2.2 `ModelPort` interface — `complete({ system?, messages, images?, serverTools?, model?,
...
- [ ] 2.4 `createAnthropicModelPort()` — wraps `@anthropic-ai/sdk`, server-only, import-guarded;
...
- [ ] 3.1 `Tool` contract — `{ name, title, inputSchema, outputSchema, run(ctx) }`;
...
- [ ] 3.2 Tool registry (`src/tools/registry.ts`) — `name → Tool`; resolve-by-name; the surface
      and future auto-trigger/graph editor read it
- [ ] 3.3 `runTool(toolName, { projectId, input, source })` on the `TenantDb` seam: resolve from
...
- [ ] 3.4 On `run`/validation failure: record a `tool_run` with status `error`; no suggestion
      or post; surface the error. Enforce a step-budget cap on `source: "auto" | "compose"` runs
- [ ] 3.5 The reference/echo tool — `{ note }` in; reads the snapshot; asks the **mock** port
...
- [ ] 3.6 Unit tests: contract validates input + `output`; runner emits deduped pending
