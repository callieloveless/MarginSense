...
optional image input, and optional server-tool declarations, and its response SHALL report
content, token usage, and optional citations — so tools that add vision or web search extend
the port's behavior rather than its interface. The port SHALL have a mock implementation used
by unit tests and a real Anthropic implementation gated behind `ANTHROPIC_API_KEY`; when the
key is absent the system SHALL report the AI layer as unconfigured and SHALL NOT construct the
real client.
...
status, and source — through the tenant-scoped data layer, including runs that fail. Each
`tool_run` row SHALL carry a non-null `business_id` with Row-Level Security enforced. The
...
- **THEN** a `tool_run` row records its tool name, project, token usage, latency, source, and a
  status of `ok`, and the suggestion and message reference that `tool_run`
...
- **THEN** a `tool_run` row is still recorded with status `error` and whatever usage and latency
  accrued, and no suggestion or message is created
