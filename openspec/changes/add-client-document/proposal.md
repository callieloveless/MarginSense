...
- **The document is a snapshot of only client-safe fields.** A `src/document/` module (pure,
  ...
  total, and terms. It **has no field for** cost, overhead, contingency, margin, EPH, the signal
  colour, or labor minutes. The sensitive numbers are never written into the row, so they cannot
  leak — a leak isn't guarded against, it's unrepresentable. Because it's a stored snapshot, a
  later edit to the estimate never silently changes what a client was already sent.
- **A public share link, revocable, behind a token.** Sharing a document mints a high-entropy
  token and sets `shared_at`; the client opens `/share/<token>` — **no login** — and sees only the
  stored payload. The read runs through a `SECURITY DEFINER` function
  (`public.get_shared_document(token)`) that returns the payload **only** when the token matches a
  document that is shared and not revoked — the single deliberate public capability (constitution
  §7: sharing is a deliberate, user-initiated action), with the table's RLS unchanged for every
  authenticated path. Revoking sets `revoked_at`; the link then shows "no longer available".
...
- **The `DocumentBackend` seam** on `TenantDb` (memory impl for isolation tests; Drizzle impl
  ...
  payload**; building that payload from an estimate is 10b's job.
- **No tool, no AI, no estimate transform.** 10a ships the table, the sharing security, the render,
  and the seam — provable end to end from a hand-built payload. 10b adds the Client Estimate Doc
  **tool** that projects an estimate into the payload and (optionally) writes a scope narrative
  with the model.
...
- **New code:** `src/document/` (the pure client-safe payload schema + validation + display
  ...
  **and** the `get_shared_document` `SECURITY DEFINER` function (+ `GRANT EXECUTE` to `anon` so the
  ...
  `app/share/[token]/page.tsx` render (outside the auth-gated `(app)` group); share/revoke server
  actions; unit tests (payload validation rejects any non-safe field; in-memory tenant isolation:
  cross-tenant read/share/revoke blocked, `business_id` stamped from the handle; the token function
  returns a shared doc's payload, and returns nothing for a wrong, unshared, or revoked token).
