...
- [ ] 1.1 Schema: `tool_runs.status` → nullable (null = running); add nullable `completed_at`.
      Update `ToolRunRow` types; add a `runStatus(row) = row.status ?? "running"` helper.
- [ ] 1.2 `npm run db:generate` → migration `0005` (`ALTER COLUMN status DROP NOT NULL` +
      `ADD COLUMN completed_at`); confirm both statements; no RLS change needed (0004's policy
      already covers the table).
- [ ] 1.3 `ToolRunsBackend`: replace `insert` usage with `startRun` (insert running) +
      `finalizeRun(businessId, id, patch)` (update status/tokens/latency/completed_at). Memory
      + Drizzle impls.
- [ ] 1.4 `TenantDb`: `recordToolRun` → `startToolRun({projectId, toolName, source})` +
...
- [ ] 1.5 Update the tool-runs isolation test for the lifecycle (start → complete; cross-tenant
      read/finalize blocked; business_id from the handle).

## 2. Dispatch seam + runner refactor (`src/tools/`)

- [ ] 2.1 Define `DispatchRequest = { toolName, projectId, input, source?, step? }` and
...
- [ ] 2.2 `dispatch(request, deps)`: resolve tool (unknown → reject, no run) → enforce step
...
- [ ] 2.3 Refactor `runTool` into the dispatch core (lifecycle ports); keep dedup, linked
      suggestions/post, and the read-only boundary intact.
- [ ] 2.4 Trigger registry `TRIGGERS: Record<string, readonly string[]>` (ships empty) +
      `emit(event, ctx, deps)` dispatching each subscriber with `source:"auto"`,
      `step: (ctx.step??0)+1`.
- [ ] 2.5 Unit tests: dispatch starts+finalizes a run; unknown tool → no run; error path
...
- [ ] 3.1 `app/_lib/tool-runner.ts`: adapter exposes the lifecycle ports + `buildSnapshot`
      binding over `TenantDb` + the snapshot assembler; provide a `dispatchForSession(...)`
      helper.
- [ ] 3.2 Re-point `app/(app)/projects/[id]/tools/actions.ts` at `dispatch` (source `user`);
...
- [ ] 4.2 `src/tools/` stays framework/DB-free (persists only through injected ports); `src/`
      relative imports extensionless; no queue/background infra introduced.
- [ ] 4.3 Dispatch is the only tool-invocation path; the trigger registry ships empty and
      `emit` is a proven no-op; lifecycle finalization is asserted for ok and error.
- [ ] 4.4 `openspec validate add-tool-dispatch --strict` passes.
