...
- [ ] 1.1 Add nullable columns to `projects` in `src/db/schema.ts` (`job_type` text, `crew_size`
...
- [ ] 1.2 Thread the fields through `createProject` (row types + `tenant.ts`/`drizzle-backend.ts`
  input) and `projectInputSchema` (`src/db/validation.ts`) — optional, percentages → basis points at
  the boundary.
- [ ] 1.3 **Isolation test:** extend the in-memory projects isolation test to write/read the new
...
- [ ] 2.1 On estimate creation, seed `target_margin_bp` / `contingency_bp` from the project default
...
- [ ] 3.1 Replace the inline new-project form with a 2-step wizard on the shell primitives
...
- [ ] 3.2 Wire the wizard to `createProjectAction`; on success land on the project page (the rich hub
  is the next change); phone-first, chips over typing.

## 4. Verify & isolation guard

- [ ] 4.1 `npm run typecheck`, `npx vitest run`, `npm run build`; apply the migration to the live DB
  and prove the new fields are tenant-scoped; phone-width walk of create-job (incl. the minimal
  client-name-only path).
- [ ] 4.2 Update `PROGRESS.md` R4 status (note the hub follows as `revamp-project-hub`).
