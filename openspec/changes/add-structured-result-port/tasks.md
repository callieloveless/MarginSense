...
- [ ] 1.1 Add optional `resultSchema` (Zod) to `ModelRequest` and optional `result` (validated
...
- [ ] 1.2 A small helper to parse/narrow `response.result` with the caller's `resultSchema`
      (typed at the call site).

## 2. Mock (`src/ai/mock.ts`)

- [ ] 2.1 When `resultSchema` is present, return a canned value validated through the schema +
      a couple of citations; deterministic, offline. Keep the `result`/`usage` mock options.
- [ ] 2.2 Unit tests: mock returns a schema-valid `result` + citations; a scripted `result`
      round-trips; no network.

## 3. Real Anthropic impl (`src/ai/anthropic.ts`)

- [ ] 3.1 Install `@anthropic-ai/sdk`.
- [ ] 3.2 `createAnthropicModelPort`: messages call with `web_search_20260209` (when declared) +
...
- [ ] 3.3 Unit tests: resolver still reports `unconfigured` without a key and does not construct
...
- [ ] 4.2 `src/tools/`/`src/engine/`/`src/context/` stay free of `@anthropic-ai/sdk`; only the
      real `src/ai/` impl imports it; the mock path builds without exercising it; `src/` relative
      imports extensionless.
- [ ] 4.3 `openspec validate add-structured-result-port --strict` passes.
