...
- [ ] 1.2 Implement `dispatchAndCompose(request, { tenantDb, port })`: build `dispatchDeps(tenantDb,
      ...
      compose nothing.
- [ ] 1.3 Make the fan-out failure-isolating: a throwing consumer, an unknown consumer name, a
      ...
      already succeeded.
- [ ] 1.4 Do not raise `MAX_TOOL_STEPS` or change `dispatch`/the runner; the entry feeds the step
      budget the right step, not a wider one.

## 2. Route the app-layer tool actions through it (no behavior change)

- [ ] 2.1 Replace the raw `dispatch(...)` in `app/_lib/photo-advise.ts` with
      `dispatchAndCompose(...)`, passing the resolved `tenantDb` and `port`. With `COMPOSE_EDGES`
      empty the result is identical.
- [ ] 2.2 Do the same in Material Finder's search action and the reference tool's action, so every
      app-layer tool run shares one dispatch entry. Leave the manual-add path (no model, no
      dispatch) untouched.
- [ ] 2.3 Confirm no result message or user-visible behavior changes today (no edge → nothing to
...
- [ ] 3.2 Assert the step boundary: a producer at step 0 yields composed runs at step 1; and a
...
- [ ] 3.4 Assert failure isolation: a mapper that throws and a consumer that throws are both logged
      and do not throw out of `dispatchAndCompose`, and the producer's outcome is still returned.
- [ ] 3.5 Assert the dormant default: with `COMPOSE_EDGES` empty, `dispatchAndCompose` starts no
